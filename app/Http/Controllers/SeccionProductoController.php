...
class SeccionProductoController extends Controller
{
	...
	{	
		...
		$subfamilia            = Subfamilia::find($producto->subfamilia_id);
		$familia               = ($producto->subfamilia_id > '1')?Familia::find($subfamilia->familia_id):Familia::find($producto->familia_id);
		$subfamilias_productos = Producto::where('familia_id', $familia->id)->where('subfamilia_id', $subfamilia->id)->orderBy('orden', 'asc')->get();	
		$activo                = ($subfamilia->id > '1')?$subfamilia->id:$producto->id;
		$productos_sf 	 	   = Producto::where('familia_id', $producto->familia_id)->get();


	    return view('page.productos.showProducto', compact('productos_sf', 'familias', 'seccion', 'metadato', 'familia', 'productos', 'activo', 'subfamilias','subfamilia', 'subfamilias_productos', 'producto'));
	}
